...
export function GoldenOrb({ state }: Props) {
  ...
  useEffect(() => {
    ...
    // Wireframe overlay
    const wireMat = new THREE.MeshBasicMaterial({ color: '#FFD700', wireframe: true, transparent: true, opacity: 0.25 })
    scene.add(new THREE.Mesh(geometry, wireMat))

    // Outer glow
    const glowGeo = new THREE.SphereGeometry(4.8, 32, 32)
    const glowMat = new THREE.MeshBasicMaterial({ color: '#FFD700', transparent: true, opacity: 0.06, side: THREE.BackSide })
    ...
    scene.add(keyLight)
    const frontLight = new THREE.PointLight(0xffd700, 2, 20)
