...
export function GoldenOrb({ state }: Props) {
  ...
  useEffect(() => {
    ...
    // Wireframe overlay
    const wireMat = new THREE.MeshBasicMaterial({ color: '#a5b4fc', wireframe: true, transparent: true, opacity: 0.3 })
    scene.add(new THREE.Mesh(geometry, wireMat))

    // Outer glow
    const glowGeo = new THREE.SphereGeometry(4.8, 32, 32)
    const glowMat = new THREE.MeshBasicMaterial({ color: '#6366f1', transparent: true, opacity: 0.08, side: THREE.BackSide })
    ...
    scene.add(keyLight)
    const frontLight = new THREE.PointLight(0x6366f1, 2.5, 20)
